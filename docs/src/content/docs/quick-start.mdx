...
<Tabs>
  ...
  <TabItem label="SQLite">
    ...
    dbcrust ./path/to/db.sqlite      # scheme inferred from extension
    ```
  </TabItem>
  <TabItem label="ClickHouse">
    ...
    # HTTPS (ClickHouse HTTP TLS port, usually 8443)
    dbcrust clickhouse://user:pass@host:8443/default

    # HTTPS on a custom HTTP port (explicit TLS via query parameter)
    dbcrust clickhouse://user:pass@host:9443/default?ssl=true

    # Note: port 9440 is the native clickhouse-client TLS port, not HTTP(S)
    ```
  </TabItem>
  ...
  <TabItem label="Files">
    ```bash
    dbcrust ./data/sales.parquet               # scheme inferred
    dbcrust './logs/*.csv?header=true'         # globs work too
    dbcrust file://                            # interactive compatible-file picker
    dbcrust json:///events.json                # explicit schemes still work
    ```
  </TabItem>
