...
class PatternDetector:
    ...
    def _is_n_plus_one_pattern(self, pattern: str, queries: List[CapturedQuery]) -> bool:
        ...
        # Django emits quoted, table-qualified columns with %s placeholders
        # (WHERE "app_book"."author_id" = %s, backticks on MySQL). Strip the
        # quoting and qualification first — the raw \w-based patterns never
        # matched real ORM SQL.
        pattern_upper = pattern.upper()
        normalized = re.sub(r'["`]', '', pattern_upper)
        normalized = re.sub(r'\b\w+\.', '', normalized)  # drop "table." prefixes

        # Common N+1 shapes; placeholder may be ?, %s, or an inlined value
        n_plus_one_patterns = [
            r'WHERE\s+\w*_?ID\s*=\s*(\?|%S|\d+)',  # WHERE user_id = %s / id = ?
            r'WHERE\s+\w+\s+IN\s*\(',              # WHERE id IN (%s, %s)
        ]

        return any(re.search(p, normalized) for p in n_plus_one_patterns)
    ...
    def _is_foreign_key_lookup(self, query1: CapturedQuery, query2: CapturedQuery) -> bool:
        ...
        # Require an FK-shaped predicate: the old ('WHERE' and 'ID') check
        # matched every Django SELECT (they all contain "id"), flagging every
        # consecutive query pair as missing select_related
        normalized = re.sub(r'["`]', '', query2.sql.upper())
        if not re.search(r'\b\w+_ID\b\s*(=|IN\b)', normalized):
            return False
    ...
    def _is_count_only_pattern(self, query: CapturedQuery) -> bool:
        ...
        # Whether the result is only *used* for counting cannot be inferred
        # from SQL. The old check (broken operator precedence) flagged every
        # SELECT without LIMIT; restrict to unfiltered, unlimited full-table
        # fetches — the only shape where the suspicion is plausible.
        sql_upper = query.sql.upper()
        fetches_everything = 'SELECT *' in sql_upper or 'SELECT "' in query.sql
        return (fetches_everything
    ...
    def _extract_foreign_key_fields(self, queries: List[CapturedQuery]) -> List[str]:
        ...
        for query in queries:
            ...
            # Pattern: WHERE table_field_id = ? — the _ID suffix must be part
            # of the capture group (it used to be outside, so the
            # endswith('_ID') check never matched and fields degraded to
            # rstrip garbage)
            import re
            patterns = [
                r'WHERE\s+["`]?\w+["`]?\.["`]?(\w+_ID)["`]?\s*=',  # table.field_id =
                r'WHERE\s+["`]?(\w+_ID)["`]?\s*=',                 # field_id =
                r'INNER\s+JOIN\s+["`]?\w+["`]?\s+ON.*["`]?\w+["`]?\.["`]?(\w+_ID)["`]?',  # JOIN
            ]

            for pattern in patterns:
                for match in re.findall(pattern, sql_upper, re.IGNORECASE):
                    field = match[:-3].lower().lstrip('"`')
            ...
            for query in queries[1:]:  # Skip first query
                for table in query.table_names:
                    if table not in [t for q in queries[:1] for t in q.table_names]:
                        ...
                        # Strip a single plural 's' — rstrip('s') removes ALL
                        # trailing s chars ('address' became 'addre')
                        field = table[:-1] if table.endswith('s') else table
