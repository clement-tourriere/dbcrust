...
class DjangoProjectAnalyzer:
    ...
    def analyze_models_only(self) -> List[DjangoModel]:
        ...
        return list(self._analyze_models().values())

    def suggest_indexes(self) -> Dict[str, List[List[str]]]:
        """
        Suggest database indexes based on model analysis.

        Returns:
            Dict mapping model names to suggested indexes (one field list per index)
        """
...
""")
    ...
    def _parse_field_definition(self, node: ast.AST) -> Dict[str, Any]:
        ...
        result: Dict[str, Any] = {'type': None, 'related_model': None, 'db_index': False}
        ...
        if isinstance(node, ast.Call):
            ...
            for keyword in node.keywords:
                if keyword.arg == 'db_index' and isinstance(keyword.value, ast.Constant):
                    result['db_index'] = bool(keyword.value.value)
