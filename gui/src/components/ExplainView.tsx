...
function getNodeStyle(detail: string): {
  ...
  if (d.includes("SCAN") && !d.includes("INDEX")) {
    return { icon: Search, color: "text-amber-400", bgColor: "bg-amber-500/10", label: "Table Scan" };
  }
  if (d.includes("INDEX") || d.includes("USING")) {
    return { icon: Zap, color: "text-emerald-400", bgColor: "bg-emerald-500/10", label: "Index Scan" };
  }
  if (d.includes("SEARCH")) {
    return { icon: Search, color: "text-blue-400", bgColor: "bg-blue-500/10", label: "Search" };
  }
  if (d.includes("MERGE") || d.includes("JOIN")) {
    return { icon: TreePine, color: "text-purple-400", bgColor: "bg-purple-500/10", label: "Join" };
  }
  if (d.includes("SORT") || d.includes("ORDER")) {
    return { icon: ArrowRight, color: "text-cyan-400", bgColor: "bg-cyan-500/10", label: "Sort" };
  }
  if (d.includes("AGGREGATE") || d.includes("GROUP")) {
    return { icon: Database, color: "text-pink-400", bgColor: "bg-pink-500/10", label: "Aggregate" };
  }
  return { icon: Info, color: "text-zinc-400", bgColor: "bg-zinc-800", label: "Step" };
}
...
export function ExplainView({ results }: ExplainViewProps) {
  ...
  const usesIndex = steps.some(
    (s) => s.detail.toUpperCase().includes("INDEX") || s.detail.toUpperCase().includes("USING"),
  );
  ...
  const gradeBg = score >= 80
    ? "bg-emerald-500/10 border-emerald-500/20"
    : score >= 60
      ? "bg-amber-500/10 border-amber-500/20"
      : "bg-red-500/10 border-red-500/20";

  /** Raw plan text (for pasting into Dalibo, depesz, etc.) */
  const planResultText = useMemo(() => {
    if (!results.rows.length) return "";
    if (results.columns.length === 1) {
      return results.rows.map((r) => r[0]).join("\n");
    }
  ...
  const copyAnalysis = () => {
    const lines: string[] = [
      ...
      "",
      "Plan:",
      ...steps.map((s, i) => `${"  ".repeat(s.indent)}${i + 1}. ${s.detail}`),
    ];
  ...
  return (
    <div className="h-full overflow-auto p-4 space-y-4">
      ...
      <div className="flex items-center gap-4">
        <div className={`flex items-center gap-3 px-4 py-3 rounded-xl border ${gradeBg}`}>
          ...
          <div>
            ...
            <div className="text-xxs text-zinc-500">
              {score >= 80 ? "Good performance" : score >= 60 ? "Room for optimization" : "Needs optimization"}
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="text-xs text-zinc-500">
            {steps.length} step{steps.length !== 1 ? "s" : ""} &middot; {results.elapsed_ms}ms
          </div>
        </div>
        ...
        <div className="ml-auto flex items-center gap-2">
          <button
            onClick={() => copyTo(planResultText, "plan")}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium
              bg-accent hover:bg-accent-hover text-white transition-all"
            title="Copy raw plan output for explain.dalibo.com, depesz, etc."
          >
            {copiedWhat === "plan" ? (
          ...
          <button
            ...
            title="Copy human-readable analysis summary"
          >
            {copiedWhat === "analysis" ? (
            ...
            return (
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className={`w-7 h-7 rounded-lg ${style.bgColor} flex items-center justify-center flex-shrink-0`}>
                    <NodeIcon className={`w-3.5 h-3.5 ${style.color}`} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className={`text-xxs font-semibold uppercase tracking-wider ${style.color}`}>
                        {style.label}
                      </span>
                      <span className="text-xxs text-zinc-600">Step {i + 1}</span>
                    </div>
                    <div className="text-sm text-zinc-200 font-mono mt-0.5">{step.detail}</div>
                    {warnings.map((w, wi) => (
                      <div key={wi} className="flex items-center gap-1.5 mt-1.5 text-xxs text-amber-400">
      ...
      {hasWarnings && (
        <div className="bg-surface rounded-xl border border-zinc-800 overflow-hidden">
          ...
          <div className="p-4 space-y-2">
            {steps
              .flatMap((s) => getWarnings(s.detail).map((w) => ({ step: s, warning: w })))
              .map(({ step, warning }, i) => (
            ...
            <thead>
              <tr>
                {results.columns.map((col, i) => (
                  <th key={i} className="text-left px-2 py-1.5 text-zinc-500 font-semibold border-b border-zinc-800">
                    {col}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {results.rows.map((row, ri) => (
                <tr key={ri} className="border-b border-zinc-800/30 hover:bg-zinc-800/20">
                  {row.map((cell, ci) => (
                    <td key={ci} className="px-2 py-1.5 text-zinc-300">{cell}</td>
                  ))}
                </tr>
              ))}
            </tbody>
