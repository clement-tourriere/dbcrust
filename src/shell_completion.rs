...
fn generate_bash_completion<W: Write>(
    ...
    // Find the main completion function and add our custom URL completion logic
    let custom_functions = r#"
# DBCrust URL scheme completion
_dbcrust_complete_url_schemes() {
    local schemes=(
"#;
    ...
    scheme_list.push_str(r#"
_dbcrust_complete_docker_containers() {
    local containers
    if command -v docker &> /dev/null; then
        # Get running database containers
        containers=$(docker ps --format '{{.Names}}' | grep -E 'postgres|mysql|mariadb|sqlite' 2>/dev/null || true)
...
_dbcrust_complete_sessions() {
    local sessions
    local config_file="$HOME/.config/dbcrust/sessions.toml"
    if [[ -f "$config_file" ]]; then
        # Extract session names from TOML file
        sessions=$(grep '^\[saved_sessions\.' "$config_file" 2>/dev/null | sed 's/\[saved_sessions\.\(.*\)\]/\1/' || true)
        if [[ -n "$sessions" ]]; then
            printf '%s\n' $sessions
        fi
    fi
...
_dbcrust_complete_url() {
    local cur="$1"
    
    # If the current word doesn't contain "://", complete schemes
    if [[ "$cur" != *"://"* ]]; then
        _dbcrust_complete_url_schemes | grep "^$cur"
    else
        ...
        case "$scheme" in
            docker)
                ...
                local containers=$(_dbcrust_complete_docker_containers)
                if [[ -n "$containers" ]]; then
            ...
            session)
                ...
                local sessions=$(_dbcrust_complete_sessions)
                if [[ -n "$sessions" ]]; then
...
"#);
    ...
    let insertion_point = completion_script.find("_dbcrust() {").unwrap_or(0);
    ...
    if let Some(pos) = completion_script.find("local context;") {
        let insert_code = r#"
            ...
            if [[ ${cur} == -* ]] || [[ ${argc} -eq 0 ]]; then
                ...
                if [[ ${argc} -eq 0 ]] || [[ ${prev} == "dbcrust" ]] || [[ ${prev} == "dbc" ]]; then
                    local url_completions=$(_dbcrust_complete_url "$cur")
...
fn generate_zsh_completion<W: Write>(
    ...
    // Add custom URL completion functions
    let custom_functions = r#"
# DBCrust URL scheme completion functions
_dbcrust_url_schemes() {
    local schemes=(
"#;
    
    let mut scheme_functions = String::from(custom_functions);
    for scheme in UrlScheme::iter() {
        scheme_functions.push_str(&format!("        '{}:{}'\n", scheme.url_prefix(), scheme.description()));
    }
    scheme_functions.push_str(r#"    )
    _describe 'url schemes' schemes
}

_dbcrust_docker_containers() {
    local containers
    if (( $+commands[docker] )); then
        containers=(${(f)"$(docker ps --format '{{.Names}}' | grep -E 'postgres|mysql|mariadb|sqlite' 2>/dev/null || true)"})
        if [[ -n "$containers" ]]; then
...
_dbcrust_sessions() {
    ...
    local config_file="$HOME/.config/dbcrust/sessions.toml"
    if [[ -f "$config_file" ]]; then
        sessions=(${(f)"$(grep '^\[saved_sessions\.' "$config_file" 2>/dev/null | sed 's/\[saved_sessions\.\(.*\)\]/\1/' || true)"})
        if [[ -n "$sessions" ]]; then
...
"#);
    ...
    // Insert custom functions at the beginning
    completion_script.insert_str(0, &scheme_functions);
    
    // Modify the arguments section to include URL completion
    if let Some(pos) = completion_script.find("'::url -- Database connection URL:'") {
        let end_pos = completion_script[pos..].find('\n').unwrap_or(0) + pos;
        completion_script.replace_range(
            pos..end_pos,
            "'::url -- Database connection URL:_dbcrust_complete_url'",
        );
    }
...
function __dbcrust_sessions
    ...
    if test -f "$config_file"
        grep '^\[saved_sessions\.' "$config_file" 2>/dev/null | sed 's/\[saved_sessions\.\(.*\)\]/\1/'
    end
end
