...
impl Database {
    ...
    pub async fn new(
        ...
        Self::from_connection_info(connection_info, default_limit, expanded_display_default, ssh_tunnel_config).await
    }
    ...
    pub fn get_username(&self) -> String {
        if let Some(ref client) = self.database_client {
            client.get_connection_info().username.clone().unwrap_or_else(|| "unknown".to_string())
        } else {
            "unknown".to_string()
        }
    }
    ...
    pub fn get_host(&self) -> String {
        if let Some(ref client) = self.database_client {
            client.get_connection_info().host.clone().unwrap_or_else(|| "unknown".to_string())
        } else {
            "unknown".to_string()
        }
    }
    ...
    pub async fn list_tables(
        ...
        if let Some(ref database_client) = self.database_client {
            let tables = database_client.get_metadata_provider().get_tables(None).await?;
    ...
    pub fn maybe_add_limit(&self, query: &str) -> String {
        ...
        if self.default_limit > 0 && !query_lower.contains("limit") && (query_lower.contains("select") && !query_lower.contains("count(")) {
    ...
    pub async fn list_database_names(
        ...
        if let Some(ref database_client) = self.database_client {
            ...
            let names: Vec<String> = databases.into_iter().skip(1).map(|row| row[0].clone()).collect();
            return Ok(names);
        } else {
            return Err("No database client available".into());
        }
    ...
    pub async fn get_functions(
        ...
        // Use new database abstraction layer
        if let Some(ref database_client) = self.database_client {
            let functions = database_client.get_metadata_provider().get_functions(schema_filter).await?;
            return Ok(functions);
        } else {
            return Err("No database client available".into());
        }
    ...
    pub async fn get_columns_for_table(
        ...
        // Use new database abstraction layer
        if let Some(ref database_client) = self.database_client {
            let columns = database_client.get_metadata_provider().get_columns(table_name, schema).await?;
            return Ok(columns);
        } else {
            return Err("No database client available".into());
        }
