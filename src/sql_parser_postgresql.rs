...
impl PostgreSQLParser {
    ...
    fn detect_postgresql_patterns(&self, sql: &str) -> DatabaseSpecificContext {
        ...
        has_window_functions = window_keywords.iter().any(|keyword| {
            sql.to_uppercase().contains(keyword)
        });
        
        // Check for CTEs
        has_cte = sql.to_uppercase().contains("WITH") && 
...
impl SqlParserEngine for PostgreSQLParser {
    ...
    fn get_keywords_by_category(&self, category: KeywordCategory) -> Vec<&'static str> {
        match category {
            ...
            KeywordCategory::DML => vec![
                "SELECT", "INSERT", "UPDATE", "DELETE", "MERGE", "COPY", "RETURNING"
            ],
            KeywordCategory::Functions => vec![
                "COALESCE", "NULLIF", "GREATEST", "LEAST", "JSON_BUILD_OBJECT",
                "JSON_AGG", "ARRAY_AGG", "STRING_AGG", "UNNEST", "GENERATE_SERIES"
            ],
            KeywordCategory::Operators => vec![
                "AND", "OR", "NOT", "IN", "EXISTS", "BETWEEN", "LIKE", "ILIKE",
                "SIMILAR", "TO", "IS", "DISTINCT", "FROM"
            ],
            KeywordCategory::DataTypes => vec![
                "BIGINT", "INTEGER", "SMALLINT", "DECIMAL", "NUMERIC", "REAL",
                "DOUBLE", "PRECISION", "SERIAL", "BIGSERIAL", "MONEY", "TEXT",
                "VARCHAR", "CHAR", "BYTEA", "TIMESTAMP", "TIMESTAMPTZ", "DATE",
                "TIME", "TIMETZ", "INTERVAL", "BOOLEAN", "UUID", "JSON", "JSONB",
                "ARRAY", "INET", "CIDR", "MACADDR", "POINT", "LINE", "BOX",
                "PATH", "POLYGON", "CIRCLE"
            ],
            KeywordCategory::SystemFunctions => vec![
                "NOW", "CURRENT_DATE", "CURRENT_TIME", "CURRENT_TIMESTAMP",
                "CURRENT_USER", "SESSION_USER", "USER", "CURRENT_CATALOG",
                "CURRENT_SCHEMA", "VERSION", "PG_BACKEND_PID"
            ],
            KeywordCategory::AggregateFunctions => vec![
                "COUNT", "SUM", "AVG", "MAX", "MIN", "ARRAY_AGG", "STRING_AGG",
                "JSON_AGG", "JSONB_AGG", "BOOL_AND", "BOOL_OR", "BIT_AND", "BIT_OR",
    ...
    fn get_functions(&self) -> Vec<&'static str> {
        vec![
            ...
            "CEIL", "FLOOR", "COALESCE", "NULLIF", "GREATEST", "LEAST",
            
            // PostgreSQL-specific functions
            "STRING_AGG", "ARRAY_AGG", "UNNEST", "GENERATE_SERIES",
            "JSON_BUILD_OBJECT", "JSON_BUILD_ARRAY", "JSON_AGG", "JSONB_AGG",
            "JSONB_BUILD_OBJECT", "JSONB_BUILD_ARRAY", "JSON_EXTRACT_PATH",
            "JSON_EXTRACT_PATH_TEXT", "JSONB_EXTRACT_PATH", "JSONB_EXTRACT_PATH_TEXT",
            "AGE", "EXTRACT", "DATE_PART", "DATE_TRUNC", "TO_CHAR", "TO_DATE",
            "TO_TIMESTAMP", "TO_NUMBER", "FORMAT", "LEFT", "RIGHT", "REVERSE",
            "TRANSLATE", "OVERLAY", "REGEXP_REPLACE", "REGEXP_SPLIT_TO_ARRAY",
            "ARRAY_APPEND", "ARRAY_PREPEND", "ARRAY_CAT", "ARRAY_LENGTH",
    ...
    fn get_data_types(&self) -> Vec<&'static str> {
        vec![
            ...
            "TEXT", "VARCHAR", "CHAR", "BYTEA", "TIMESTAMP", "TIMESTAMPTZ",
            "DATE", "TIME", "TIMETZ", "INTERVAL", "BOOLEAN", "UUID", "JSON",
            "JSONB", "ARRAY", "INET", "CIDR", "MACADDR", "MACADDR8", "POINT",
            "LINE", "LSEG", "BOX", "PATH", "POLYGON", "CIRCLE", "BIT", "VARBIT",
            "TSVECTOR", "TSQUERY", "XML", "PG_LSN", "TXID_SNAPSHOT",
        ]
    }
        ...
        match context.base_context.current_clause {
            ...
            SqlClause::From => {
                ...
                matches!(upper_keyword.as_str(),
                    ...
                    "CROSS" | "LATERAL" | "ON" | "USING" | "WHERE" | "GROUP" |
            ...
            SqlClause::Where => {
                ...
                matches!(upper_keyword.as_str(),
                    ...
                    "LIKE" | "ILIKE" | "SIMILAR" | "TO" | "IS" | "NULL" |
                    "DISTINCT" | "FROM" | "ANY" | "SOME" | "ALL" |
...
mod tests {
    ...
    fn test_postgresql_window_function_detection() {
        ...
        if let DatabaseSpecificContext::PostgreSQL { has_window_functions, .. } = context {
