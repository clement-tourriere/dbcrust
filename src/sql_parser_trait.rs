...
use crate::sql_parser::{SqlContext, SqlClause};
...
impl SqlParserFactory {
    ...
    pub fn create_parser(database_type: DatabaseType) -> Box<dyn SqlParserEngine> {
        match database_type {
            DatabaseType::PostgreSQL => Box::new(crate::sql_parser_postgresql::PostgreSQLParser::new()),
...
impl SqlParserEngine for GenericSqlParser {
    ...
    fn get_operators(&self) -> Vec<&'static str> {
        vec!["=", "!=", "<>", "<", ">", "<=", ">=", "AND", "OR", "NOT", "IN", "LIKE", "BETWEEN"]
    }
    
    fn get_data_types(&self) -> Vec<&'static str> {
        vec!["TEXT", "INTEGER", "REAL", "BOOLEAN", "DATE", "TIME", "TIMESTAMP"]
    }
    ...
    fn get_context_suggestions(&self, _context: &EnhancedSqlContext, _current_word: &str) -> Vec<String> {
...
pub mod parsing_utils {
    ...
    pub fn needs_quoting(identifier: &str, database_type: DatabaseType) -> bool {
        ...
        match database_type {
            DatabaseType::PostgreSQL => {
                matches!(upper_identifier.as_str(), "SELECT" | "FROM" | "WHERE" | "ORDER" | "GROUP")
            }
            DatabaseType::MySQL => {
                matches!(upper_identifier.as_str(), "SELECT" | "FROM" | "WHERE" | "ORDER" | "GROUP" | "LIMIT")
            }
            DatabaseType::SQLite => {
                matches!(upper_identifier.as_str(), "SELECT" | "FROM" | "WHERE" | "ORDER" | "GROUP")
            }
