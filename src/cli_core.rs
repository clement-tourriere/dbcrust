...
impl CliCore {
    ...
    pub async fn run_with_args(args: Args) -> Result<i32, CliError> {
        ...
        if !args.command.is_empty() {
            if cli_core.can_handle_commands_without_connection(&args.command) {
        ...
        if let Some(_) = &args.connection_url {
    ...
    async fn handle_command_mode_standalone(&mut self, commands: &[String]) -> Result<(), CliError> {
        for command in commands {
            ...
            match command_trimmed {
                ...
                _ => {
                    eprintln!("Command '{}' requires a database connection", command_trimmed);
...
╚═════╝ ╚═════╝  ╚═════╝╚═╝  ╚═╝ ╚═════╝ ╚══════╝   ╚═╝
    ...
    async fn handle_vault_connection(&mut self, url: &str) -> Result<(Database, Option<ConnectionInfo>), CliError> {
        ...
        // Handle optional parameters - if None, use defaults or prompt
        let role_name = role.as_deref().unwrap_or("readonly");
        let db_name = database_name.as_deref().unwrap_or("postgres");
