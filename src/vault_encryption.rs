...
mod tests {
    ...
    fn test_encrypt_decrypt_data() {
        ...
        unsafe { std::env::set_var("VAULT_TOKEN", "test-vault-token-consistent"); }
        ...
        match original_token {
            Some(token) => unsafe { std::env::set_var("VAULT_TOKEN", token); },
            None => unsafe { std::env::remove_var("VAULT_TOKEN"); },
        }
    }
    ...
    fn test_encrypt_decrypt_string() {
        ...
        // Encrypt
        let encrypted_hex = encrypt_string(original_string).expect("String encryption should succeed");
        assert!(!encrypted_hex.is_empty());
        assert_ne!(encrypted_hex, original_string);
        
        // Decrypt
        let decrypted_string = decrypt_string(&encrypted_hex).expect("String decryption should succeed");
    ...
    fn test_no_vault_token() {
        ...
        if let Some(token) = original_token {
            unsafe { std::env::set_var("VAULT_TOKEN", token); }
        }
    }
}